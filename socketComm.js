...
exports.initSocketComm = socket => {
    console.log(`${socket.remoteAddress}:${socket.remotePort} >> CONNECTED!`);

    socket.on('close', () => console.log(`${socket.remoteAddress}:${socket.remotePort} >> Connection CLOSED!`));
    ...
    rl.log = msg => socket.write(msg);  // Add socket.write to rl interface
    ...
    rl.on('line', async (line) => {
        ...
        try{
            console.log(`${socket.remoteAddress}:${socket.remotePort} >> ${line}`);
