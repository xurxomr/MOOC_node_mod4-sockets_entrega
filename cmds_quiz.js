...
exports.create = async (rl) => {
  ...
  let name = await rl.questionSync(rl, "Enter user");
    let user = await User.findOne({where: {name}});
    if (!user) throw new Error(`User ('${name}') doesn't exist!`);

    let question = await rl.questionSync(rl, "Enter question");
    if (!question) throw new Error("Response can't be empty!");

    let answer = await rl.questionSync(rl, "Enter answer");
...
exports.test = async (rl) => {
  ...
  let answered = await rl.questionSync(rl, quiz.question);
...
exports.update = async (rl) => {
  ...
  let id = await rl.questionSync(rl, "Enter quizId");
  let quiz = await Quiz.findByPk(Number(id));

  let question = await rl.questionSync(rl, `Enter question (${quiz.question})`);
  if (!question) throw new Error("Response can't be empty!");

  let answer = await rl.questionSync(rl, `Enter answer (${quiz.answer})`);
  if (!answer) throw new Error("Response can't be empty!");
